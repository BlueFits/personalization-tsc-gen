...
export default class PuppeteerBrowser {
    ...
    constructor (page: pupeteer.Page) {
        if (typeof page === 'undefined') {
			...
			this.page = page;
			this.rootIndex = path.resolve(process.cwd() + "/dist/index-dev.html");
		}
    };
	...
	public async render() {
		...
		if (this.rootIndex) {
			file = await readFile(this.rootIndex)! as string;
		} else {
			file = path.resolve(process.cwd() + "/services/LiveReload/filler.html");
		}
